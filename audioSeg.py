...
# Utility functions
def my_read(f, normalized=False):
    """MP3 to numpy array
    return
        rate: int
        data: np array
...
def my_write(filename, rate, x, normalized=False):
    """numpy array to MP3
    Parameters
    ----------
...
def audio_split(input_file,output_dir):
    ...
    sample_rate, samples = input_data=my_read(input_filename)
    ...
    for i, start, stop in tqdm(cut_ranges):
        output_file_path = "{}_{:03d}.mp3".format(
        ...
        if not dry_run:
            ...
            # )
            my_write(output_file_path,sample_rate,samples[start:stop])
        else:
...
if __name__=="__main__":
    audio_split(sys.argv[1],sys.argv[2])
